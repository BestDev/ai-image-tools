...
若图像中有清晰可见的人物，描述：姿势（实际观察到的身体朝向与体态曲线、头部倾斜与视线方向、肩部位置、手臂及手部动作、腿部姿态与重心分布）、服装（服装类型与款式、领口形状、袖长、裙摆或裤腿长度、可见的镂空或透视区域、裸露肌肤范围、面料质感与材质如实描述、颜色与花纹、所有可见配饰包括鞋履包袋耳环项链等）、光线（可见的光源类型、阴影质感、对比度、整体氛围）、构图——根据实际可见内容标注拍摄角度（正面/轻微左右侧角/四分之三左右侧角/正侧面/过肩角度/背面/俯拍/仰拍/平视/斜角；只标注明确可辨别的角度），景别（极特写/特写/近景/中近景/半身/全身/远景），景深。
...
WARMUP_EN = (
    ...
    "arm and hand placement, leg stance, weight shift, overall silhouette and body curves), "
    ...
    "shoes, bags, jewelry, hair accessories, bracelets, anklets), "
    "lighting (source type, shadow behavior, color temperature, contrast, atmospheric mood), "
...
Pose (only if people are clearly visible): body orientation and silhouette curves as seen, head position and gaze direction as observed, arm and hand placement, leg stance and weight distribution, overall body line.
...
姿势体态（仅当人物清晰可见时）：实际观察到的身体朝向与体态曲线、头部位置与视线方向、手臂及手部动作、腿部姿态与重心分布、整体身体线条走势。
...
# ZH→EN: 중국어 지시 + 영어 출력
#
# 구현 원칙: 기존 프롬프트 마지막 출력 포맷 문장만 교체.
# 각 섹션 조건부(only if clearly visible 등)는 그대로 유지.
# 소형 모델이 이미 따르는 마지막 포맷 지시문을 교체하는 방식이므로
# 끝에 한 줄 추가하는 것보다 훨씬 안정적.
# ──────────────────────────────────────────────
_FMT_EN      = "Write a single natural English paragraph of 80–250 words in flowing prose. Do not use comma-separated tag lists."
_FMT_ZH      = "用单段流畅自然的中文输出，字数150至400字。不使用逗号分隔的标签列表。"
_FMT_SPEC_EN = "Write a single natural English paragraph of 80–250 words. Use flowing prose — do not use comma-separated tag lists. The primary subject must appear in the first sentence."
_FMT_SPEC_ZH = "用单段流畅自然的中文输出，字数150至400字。不使用逗号分隔的标签列表。主要主体必须出现在第一句话中。"

# EN 지시 본문의 "refined English prompt" → 언어 중립화
SYSTEM_PROMPT_EN_ZH = (
    SYSTEM_PROMPT_EN
    .replace(
        "Generate a refined English prompt based solely on your observations.",
        "Generate a refined prompt based solely on your observations.",
    )
    .replace(_FMT_EN, _FMT_ZH)
)

# ZH 지시 본문의 "精准的中文提示词" → 언어 중립화
SYSTEM_PROMPT_ZH_EN = (
    SYSTEM_PROMPT_ZH
    .replace(
        "仅基于实际观察生成精准的中文提示词。",
    ...
    .replace(_FMT_ZH, _FMT_EN)
)

# SPEC EN→ZH
SYSTEM_PROMPT_SPEC_EN_ZH = (
    SYSTEM_PROMPT_SPEC_EN
    .replace(
        "Generate a refined English prompt for the Z-Image Turbo text-to-image model based solely on what you observe.",
        "Generate a refined prompt for the Z-Image Turbo text-to-image model based solely on what you observe.",
    )
    .replace(_FMT_SPEC_EN, _FMT_SPEC_ZH)
)

# SPEC ZH→EN (ZH 본문 첫 문장은 이미 언어 중립)
SYSTEM_PROMPT_SPEC_ZH_EN = SYSTEM_PROMPT_SPEC_ZH.replace(_FMT_SPEC_ZH, _FMT_SPEC_EN)

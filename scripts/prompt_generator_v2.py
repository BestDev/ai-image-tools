...
from shared_prompts import (  # noqa: E402
    ...
    SYSTEM_PROMPT_EN_ZH, SYSTEM_PROMPT_ZH_EN,
    SYSTEM_PROMPT_SPEC_EN, SYSTEM_PROMPT_SPEC_ZH,
    SYSTEM_PROMPT_SPEC_EN_ZH, SYSTEM_PROMPT_SPEC_ZH_EN,
...
# ──────────────────────────────────────────────
_SYSTEM_PROMPTS = {
    ("en", "en"): {"standard": SYSTEM_PROMPT_EN,       "spec": SYSTEM_PROMPT_SPEC_EN},
    ("en", "zh"): {"standard": SYSTEM_PROMPT_EN_ZH,    "spec": SYSTEM_PROMPT_SPEC_EN_ZH},
    ("zh", "zh"): {"standard": SYSTEM_PROMPT_ZH,       "spec": SYSTEM_PROMPT_SPEC_ZH},
    ("zh", "en"): {"standard": SYSTEM_PROMPT_ZH_EN,    "spec": SYSTEM_PROMPT_SPEC_ZH_EN},
}
...
REFINE_PROMPT_EN_ZH = """Below is a detailed image description. Based on this description, generate a refined prompt for the Z-Image Turbo text-to-image model.

Requirements:
- Preserve all clothing details, pose descriptions, skin exposure, and accessories exactly as described
- Order by importance: subject → clothing & skin exposure → pose & body orientation → lighting & composition
- Write a single, natural Chinese paragraph of 150–400 characters
- Avoid references to watermarks, symbols, or irrelevant text

[Image Description]
{raw}"""

REFINE_PROMPT_ZH_EN = """以下是对图像的详细英文描述。基于此描述，生成一段精准的提示词，用于文生图模型Z-Image Turbo。

要求：
- 完整保留所有服装细节、皮肤裸露程度、姿势描述和配饰信息
- 按重要性排序：主体 → 服装与裸露区域 → 姿势体态 → 光线构图
- 用单段流畅自然的英文输出，字数80至250词
- 不提及水印、符号或无关文字
...
REFINE_PROMPT_SPEC_EN_ZH = """Below is a detailed image description. Based on this description, generate a refined prompt for the Z-Image Turbo text-to-image model following the Z-Image Turbo prompt engineering specification.

Requirements:
- Follow strict construction hierarchy: scene/environment first → subject → details → constraints
- Preserve all clothing details, pose descriptions, skin exposure, and accessories exactly as described
- Camera & lens: reproduce the camera characteristics described in the source description. Do NOT force portrait camera specs onto non-portrait subjects
- If skin is visible: describe with pore-level detail, subtle imperfections, and realistic skin translucency (subsurface scattering). Do NOT describe plastic-like smoothing
- Color palette (60-30-10 rule): identify dominant primary color (60%), secondary complementary color (30%), accent highlight (10%), and overall tonal direction
- Specify exact camera angle and shot framing
- Materials & textures: preserve actual surface characteristics — fabric materials, surface finishes, environmental textures
- Emotional sync (if people described): preserve body language, eye contact, and proximity framing. Skip if no people described
- Avoid: stock-photo aesthetic, plastic skin, oversaturated neon, harsh bloom, oversharpening, Instagram filter, watermarks
- Write a single, natural Chinese paragraph of 150–400 characters in scene→subject→details→constraints order

[Image Description]
{raw}"""

REFINE_PROMPT_SPEC_ZH_EN = """以下是对图像的详细英文描述。基于此描述，生成一段精准的提示词，用于文生图模型Z-Image Turbo，并严格遵循Z-Image Turbo提示词工程技术规范。

要求：
- 严格遵循构建层次：场景/环境优先 → 主体 → 细节 → 约束
- 完整保留所有服装细节、皮肤裸露程度、姿势描述和配饰信息
- 镜头特征：根据原始描述还原镜头特性。不得将人像镜头规格强加于非人像题材主体
- 若皮肤可见：描述毛孔级细节、细微瑕疵及真实皮肤通透感（次表面散射）。不得描述塑料质感
- 色彩搭配（60-30-10法则）：识别主导色（60%）、辅助互补色（30%）、点缀高光色（10%）
- 精确指定拍摄角度和景别
- 材质与纹理：保留原始描述中的实际表面特性
- 情感共鸣（若有人物）：保留肢体语言信号、眼神质感、亲密构图距离。若无人物则跳过
- 禁止描述：廉价图库风格、塑料质感皮肤、过度饱和色、强烈眩光、过度锐化、Instagram滤镜、水印
- 用单段流畅自然的英文输出，字数80至250词，按场景→主体→细节→约束顺序

[图像描述]
{raw}"""

_REFINE_PROMPTS = {
    ("en", "en"): {"standard": REFINE_PROMPT_EN,       "spec": REFINE_PROMPT_SPEC_EN},
    ("en", "zh"): {"standard": REFINE_PROMPT_EN_ZH,    "spec": REFINE_PROMPT_SPEC_EN_ZH},
    ("zh", "zh"): {"standard": REFINE_PROMPT_ZH,       "spec": REFINE_PROMPT_SPEC_ZH},
    ("zh", "en"): {"standard": REFINE_PROMPT_ZH_EN,    "spec": REFINE_PROMPT_SPEC_ZH_EN},
}
...
def run_qwen3vl_image(image_path: str, model, processor, lang: str = "en", uncensored: bool = False, prompt_style: str = "standard", instr_lang: str = "en") -> str:
    """이미지 직접 분석 (method 2)"""
    import torch
    from PIL import Image
    ...
    pil_image = Image.open(image_path).convert("RGB")
    prompt = _SYSTEM_PROMPTS[(instr_lang, lang)][prompt_style]
    if uncensored:
...
def run_qwen3vl_refine(raw_text: str, model, processor, lang: str = "en", uncensored: bool = False, prompt_style: str = "standard", instr_lang: str = "en") -> str:
...
def run_qwen35_image(image_path: str, model, processor, lang: str = "en", uncensored: bool = False, prompt_style: str = "standard", thinking: bool = False, instr_lang: str = "en") -> str:
    """이미지 직접 분석 (method 3)"""
    import torch
    from PIL import Image
...
def run_qwen35_refine(raw_text: str, model, processor, lang: str = "en", uncensored: bool = False, prompt_style: str = "standard", thinking: bool = False, instr_lang: str = "en") -> str:
...
def _infer_qwen3vl(img, m, p, args):
    return run_qwen3vl_image(img, m, p, lang=args.lang, uncensored=args.uncensored,
...
def _refine_qwen3vl(raw, m, p, args):
    return run_qwen3vl_refine(raw, m, p, lang=args.lang, uncensored=args.uncensored,
                              prompt_style=args.prompt_style, instr_lang=args.instr_lang)
...
# Gemini API 이미지 분석
# ──────────────────────────────────────────────
def run_gemini_image(image_path: str, client, model_id: str, lang: str = "en", prompt_style: str = "standard", instr_lang: str = "en") -> str:
...
def main():
    ...
    parser.add_argument(
        "--instr-lang", default="en", choices=["en", "zh"],
        help="지시 언어 (기본: en) — 모델에게 지시하는 언어\n"
             "지시 언어와 출력 언어를 다르게 설정해 교차 테스트 가능:\n"
             "  --instr-lang en --lang zh : 영어 지시 + 중국어 출력\n"
             "  --instr-lang zh --lang en : 중국어 지시 + 영어 출력\n"
             "method 1(JoyCaption)은 항상 en",
    )
    parser.add_argument(
    ...
    if args.method in (1,):
        lang = "en"
        instr_lang = "en"
        if args.lang != "en" or args.instr_lang != "en":
            print("※ method 1(JoyCaption)은 영어 전용입니다. --lang/--instr-lang 옵션 무시.")
    args.lang = lang
    args.instr_lang = instr_lang

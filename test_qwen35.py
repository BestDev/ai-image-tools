#!/usr/bin/env python3
"""
Qwen3.5-9B 단독 테스트 스크립트
이미지 1장을 분석하여 프롬프트를 생성하고 속도 및 품질을 확인한다.

사용법:
  python3 test_qwen35.py <이미지_경로> [--quant {nf4,bf16}]

참고:
  - 모델: Qwen/Qwen3.5-9B
  - 클래스: AutoModelForCausalLM (early fusion 네이티브 멀티모달)
  - 최신 transformers 필요 (git main 권장, 5.2.0에서 동작 여부 확인 필요)
  - Thinking mode 기본 활성화 → enable_thinking=False 비활성화 필수
  - Qwen3-VL과 달리 전용 VL 클래스 없음; AutoModelForCausalLM이 이미지 처리
"""
...
def load_model(quant: str):
    ...
    # 공식 클래스: Qwen3_5ForConditionalGeneration (VL 컴포넌트 포함)
...
def run(image_path: str, model, processor, lang: str = "en") -> str:
    ...
    # PIL Image 직접 로드 (Qwen3.5도 Qwen3-VL과 동일 방식)
    ...
    # 공식 API: apply_chat_template(return_dict=True, enable_thinking=False)
    ...
    # Tensor만 device로 이동 (non-tensor 값 제외)
    ...
    # thinking 블록 잔재 제거 (enable_thinking=False여도 혹시 남을 경우 대비)
    if "<think>" in result:
        import re
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3.5-9B 단독 테스트 (단일 이미지 또는 폴더)")
    ...
    print(f"주의   : transformers 최신 git main 권장 (5.2.0에서 VL 지원 미검증)")
    if args.output:
